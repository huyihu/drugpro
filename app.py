...
def select_all_checked():
    select_all_check = st.checkbox('SELECT ALL')
    if select_all_check:
...
tables = ['disease_drug', 'disease_target', 'drug_com_tar', 'drug_com_sim']
disease_drug_cols = ['Disease ID', 'Disease Name', 'Drug Inchikey', 'Drug Name']
...
# plot graph by user selection
graphs = ['Statistics', 'Repurposing 3D box', 'TSNE']
def plot_graphs():
    select_graph = st.selectbox('Select the graph:', graphs)
    if select_graph == 'Statistics':
        plot_statistics()
    if select_graph == 'Repurposing 3D box':
        plot_3dbox()
    if select_graph == 'TSNE':
        plot_tsne()
...
if active_tab == 'Table':
    # table selection 
    select_table = st.selectbox('Select the table:', tables)
    if select_table == 'disease_drug':
        st.dataframe(data_show[disease_drug_cols])
if active_tab == 'Graph':
    plot_graphs()
